...
Channel
    ...
    .into {reads_sub;  reads_fastqscreen ; reads_jaffa; reads_meta}
...
if (!params.subsampling) {
   ...
   .map{ row-> tuple(row.id, file(row.read1), file(row.read2)) }.view()
      ...
      process subsampling_fastqs {
        //Downsample fastqs to 50000000. To change it shall be done in the subsampling.sh script located in bin directory
	memory 70.GB 
	//when:
	//params.subsampling
	...
	output:
		set val(smpl_id) , file("${smpl_id}_read1_sub.fastq.gz"), file("${smpl_id}_read2_sub.fastq.gz") into reads_fusioncatcher,reads_align ,reads_starfusion, reads_arriba, reads_salmon, reads_dux4,reads_dux4_ch2
...
process arriba{
	...
	--alignSplicedMateMapLminOverLmate 0.5 --alignSJstitchMismatchNmax 5 -1 5 5 --chimSegmentMin 10 --chimOutType WithinBAM HardClip --chimJunctionOverhangMin 10 --chimScoreDropMax 30 --chimScoreJunctionNonGTAG 0 --chimScoreSeparation 1 --chimSegmentReadGapMax 3 --chimMultimapNmax 50
