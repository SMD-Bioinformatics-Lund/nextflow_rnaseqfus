...
def create_fastq_channel(LinkedHashMap row) {
	...
	def meta = [:]
	meta.id					= row.id
	meta.group              = (row.containsKey("group") ? row.group : row.id )
	meta.diagnosis          = row.diagnosis
	meta.type               = (row.containsKey("type") ? row.type : false)
	...
	meta.sequencing_run     = (row.containsKey("sequencing_run") ? row.sequencing_run : false)
